"""Tests for the perturbation-stability diagnostics (src.stability).

Synthetic designs only (reusing tests.test_decomposition.make_country_design)
plus synthetic country centroids: no real data, no network. The module must be
deterministic from a seed and byte-stable through round_floats, like the
...
def test_check_invariant_holds():
    ...
    # documents the invariant (bootstrap-mean shares + residual sum to 1).
    result = _summary()
    result.check()
...
def test_morans_i_degrades_when_centroids_too_few():
    ...
    # but the summary must degrade to None rather than raise.
