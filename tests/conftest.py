...
The validation and explanatory artifacts (validation_summary.json,
explain_summary.json, and so on) are written to a temporary ``processed/``
directory and passed to build_app_assets so their pages render under AppTest
without touching the real data/processed/.
"""
...
SYNTHETIC_CITIES = [
    # (city, country, lat, lon, slope in deg C per decade): six locations,
    # spread out, one above 60 N, and one same-named pair at distinct coordinates.
...
def _make_synthetic_validation_artifacts(processed: Path) -> dict[str, Path]:
    """Write minimal validation summary artifacts under `processed/`."""
    stats = {
        "n_locations": 6,
        "n_no_grid": 0,
        "n_gate_pass": 5,
...
def _make_synthetic_explain_artifacts(processed: Path) -> dict[str, Path]:
    """Write minimal explanatory summary artifacts under `processed/`.
...
def build_synthetic_bundle(root: Path) -> dict:
    ...
    write_manifest(root / "bundle", version="synthetic")
