...
residual; then, when the summary carries it, the same intervals for the
station-weighted outcome next to the area-weighted primary. The page computes
nothing; every number is precomputed by ``src.stability``, and all of it
describes the stability of a descriptive variance decomposition, not
...
def render() -> None:
    ...
    if station and share and share.get("groups"):
        ...
        st.markdown(
            "The same bootstrap on the same countries with each country's warming "
            "taken as the unweighted mean of its station trends. Point estimates "
            "with the 95% country-bootstrap interval; the last row is the residual "
            "Moran's I."
        )
        st.dataframe(_comparison_table(summary, station), width="stretch")
        st.caption(
            "Where the two intervals overlap, the share does not depend on how "
            "national warming was constructed; where they separate, it does. The "
            "gap is consistent with station-network geography but the comparison "
            "also changes the observational product (a gridded field versus "
            "station means), so it does not isolate siting as the sole cause."
        )
...
def _comparison_table(primary: dict, station: dict) -> pd.DataFrame:
    ...
    for key in keys:
        ...
        rows[f"{label} share"] = [
            _interval(g[key]) if key in g else "—" for _, g in groups
        ]
    rows["Residual Moran's I"] = [
        _num((block.get("residual_spatial") or {}).get("morans_i"))
        for _, block in columns
    ]
    rows["Countries"] = [str(block.get("n_countries", "—")) for _, block in columns]
