...
is the same decomposition on a perturbed sample, so these diagnostics speak only to
...
Area-weighted outcome (primary), 151 countries, in the original three-column
representation of the responsibility and population groups (the full-rank
intervals used on the dashboard, from `research/model_v2/outputs/m4_final/m4_bootstrap_summary.json`,
are geography 0.452–0.630, responsibility 0.006–0.035, residual 0.233–0.407):
...
- Leave-one-continent-out fits (evaluated and rejected as a validation design
  in the residual investigation: the continent effect is unidentifiable for
  the held-out block).
