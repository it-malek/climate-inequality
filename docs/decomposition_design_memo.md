...
the feature schema in `src/feature_schema.py` (`SCHEMA_V1`) and governs
...
A project like this could host **two distinct kinds of model**:
...
This decomposition is strictly the cross-sectional analysis. The
physical-driver / climate-attribution model is an **explicit non-goal** of the
decomposition and out of its scope; the global forcing regression is a separate
module and is never combined with it. The two kinds of model must stay
conceptually and statistically separated, and the decomposition is *never* to
be read as physical causality.

Two reasons make this the right boundary, not merely a convenient one:

- **The two kinds of model do not statistically compose.** A physical model
  explains *temporal* variance in a *global* temperature series; its output is a single
...
2. **City-location warming trend**: the 3,510 per-location slopes themselves.
   ...
   analysis and as an input to the country aggregate.
...
- **It is robust to the data's worst weakness.** Station sampling is wildly
  ...
  averages within national borders. (It does *not* fix the bias, see §7; it only
...
- **It is a decomposition of *explained variance*, not of *cause*.** A large
  ...
  do not absorb. See §6.
- **Groups are atomic and fixed by the schema.** Membership is part of
...
- **H5: the residual is substantial and spatially structured.** A large
  ...
  the stability diagnostics should detect significant spatial autocorrelation
