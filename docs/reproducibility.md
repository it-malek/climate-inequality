...
1. **After dependencies are installed, the dashboard and default tests need no
   raw-data download.** The app reads the committed `app/data/` bundle. Tests use
   synthetic fixtures and committed evidence; checks requiring local raw data
   explicitly skip when unavailable (`tests/conftest.py` builds a small
   end-to-end synthetic bundle through the real builder). `tests/test_artifacts.py` additionally
   checks the committed bundle on disk for internal consistency.
2. **The pipeline has deterministic rebuild checks for fixed inputs.** Every stage is a
...
Research baseline: Python 3.11 (`.python-version`); dashboard smoke coverage also
includes 3.14. The project metadata permits Python ≥ 3.11, but this is not a claim
that every research dependency has been tested on every later interpreter. Managed by [uv](https://docs.astral.sh/uv/):

```bash
uv sync --locked --extra dev          # pipeline + tests + lint
uv run --locked pytest -q
uv run --locked ruff check src tests app scripts research
```

The deployed app installs `app/requirements.txt` instead (Streamlit Community
Cloud picks the dependency file in the entrypoint's directory): a transitive, pinned export of the `dashboard` dependency group. Version constraints
live in `pyproject.toml`, resolution and artifact hashes in `uv.lock`. Regenerate with:

```bash
uv export --locked --only-group dashboard --no-hashes --no-annotate --no-header --output-file app/requirements.txt
```

CI rejects export drift. `uv sync --locked --extra notebooks` adds the archival notebook environment; it is
not needed for tests or CI. `uv sync --locked --extra era5` adds the Copernicus client for the optional
...
uv run python -m src.residual_assets # dashboard copy of the residual-investigation records
```
...
YAML mirrors of the feature schema and projection registry in `docs/`.
...
| Residual summary | `residual_assets` | `residual_structure_summary.json`, `national_warming.parquet` | values copied from `research/model_v2/outputs/` with source digests; Spearman rows recomputed from the country tables | `tests/test_artifacts.py` checks digests and recomputation |

The residual-structure investigation has its own reproduction sequence,
inputs and provenance guards, documented in
...
- **Anomalies** are taken against each location's own 1951–1980 monthly
  climatology. These are calendar-month offsets, not one constant offset. Changing the
  monthly climatology can change a monthly Theil–Sen slope; use matched anomaly
  conventions for product comparisons.
...
- **Country warming** is the unweighted mean of the location slopes (the
  ...
  intensive; cell area shrinks toward the poles). Assignment samples GPW at each
  native temperature-cell centre, not by fractional polygon intersection. Coarse
  coastlines, small islands and the products' half-degree grid offset therefore
  affect support. Coverage columns report valid/assigned cell counts, not a
  fraction of national land area.
...
- `coupling.validate_projection_frame` rejects any column outside the registered
  ...
  registered projection carries a definition hash mirrored in `docs/pcs_*.yaml`.
...
- Research records. Each stage under `research/model_v2/outputs/` carries a
  manifest with the SHA-256 of every deterministic artifact and a provenance
  file listing the digests of the code and inputs it ran from; an
  unchanged-code rerun of each outcome-facing stage was byte-identical. Those
  provenance files record the module bytes and Git commit identifiers of the
  development history in which the runs took place; in this snapshot the
  source is not byte-identical to the original scoring closure. The preserved
  historical checkout and its original execution requirements are needed for exact
  scoring; see `research/model_v2/README.md`.
...
`app/data/release.json` records SHA-256 digests for every dashboard artifact.
The entrypoint verifies the actual bytes before displaying results; missing,
changed or incompatible bundles fail explicitly. Every cached loader includes
the verified content fingerprint in its key, including loaders derived from
other loaders. Caches are in memory only. No source dataset is fetched at runtime.
The sidebar shows the first 16 hexadecimal characters of the bundle fingerprint;
Data and methods provides the full manifest as a download.

To verify the fixed bundle without raw data:

```bash
uv run python -c 'from src.snapshot import fingerprint; from src.bundle import APP_DATA_DIR; print(fingerprint(APP_DATA_DIR, require_manifest=True))'
uv run python -c 'from pathlib import Path; from src.residual_assets import build_residual_assets; build_residual_assets(Path("/tmp/climate-derived-check"))'
```

Compare the two regenerated presentation files with their committed copies.
After an intentional, verified bundle build, record its identity explicitly:

```bash
uv run python -c 'from src.snapshot import write_manifest; from src.bundle import APP_DATA_DIR; write_manifest(APP_DATA_DIR)'
```

Writing a manifest is not an independent scientific validation. The accepted
result records and their source hashes must be checked before recording a new
release. Original scoring reruns require the preserved research checkout and
its pre-specified documents, as described in `research/model_v2/README.md`.


## Public evidence and figure checks

```bash
python -m src.research_audit
uv run --locked python -m unittest discover -s app/tests -v
uv sync --locked --extra dev --extra figures
uv run --locked python -m scripts.research_summary_figure
```

The audit verifies dashboard bytes, stage manifests, the final record's citations,
and the five retained historical documents. It checks `reports/canonical_results.json`
against its sources. This is evidence verification, not raw-data reproduction.
The summary figure reads committed results only and writes to `reports/figures/`.
Historical diagnostic maps still require the original GPW and Natural Earth inputs.

`pyproject.toml` defines dependencies and groups; `uv.lock` records their resolution; `--locked` rejects stale project metadata instead
of resolving silently. `app/requirements.txt` is the generated deployment export, not a second dependency
specification; CI checks it byte-for-byte against the lock export. This repository is an application and
research workspace, not an installable library: run module commands from its root.
There is no distribution build or editable-install claim.
