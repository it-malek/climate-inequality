"""Pre-outcome redundancy diagnostic for C2 (``M1B_EVALUATION_CONTRACT.md`` §3, Amendments 2 and 3).

The M0* predictors are assembled from explicit allow-lists of predictor source columns for the
151 frozen countries, so no warming outcome, fitted value, residual or score column is ever read;
...
against its manifest digests before C2 is read, the predictor inputs against their frozen SHA-256
records, and the output records both, plus the ordered predictor frame and design identity.
...
# Predictor inputs as recorded by the M0.5 product audit (9fb56e1, outputs/product_stability_summary.json "files")
# and the M1a support record at a7dea34; asserted before any read.
...
def check_predictor_inputs(pins=None):
    """Fail closed unless every predictor input matches its frozen SHA-256 record; returns the verified digests."""
    verified = {}
...
def main(out_dir=OUTPUT_DIR):
    ...
    if features.columns.tolist() != ['iso3', 'Country', 'baseline_dryness']:
        raise ValueError('C2 features do not have the frozen columns')
