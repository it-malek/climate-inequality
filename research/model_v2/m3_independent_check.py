"""Independent reconstruction of an M3 package from its saved evidence (``M3_EVALUATOR_SPEC.md`` §4).
...
6. per family x representation x protocol, RMSE, MAE, R2, fold summaries, calibration, per-M49-region errors,
   ...
   model's committed primary out-of-fold predictions; Q1-Q5 and the final naming (§2.14), or ``arm_conditions``;
7. the accounting parts and the filtered-trend group shares (Shapley by averaging over all orderings);
8. the representation identity under Amendment A1 (theta 1e-5; predictions, scores, accounting parts 1e-6);
...
disposition are not recomputed. Written before any M3 result existed, after reading the evaluator's artifact
formats; executed after the result.
...
ACCOUNTING_TOL = 1e-9
IDENTITY_THETA_TOL = 1e-5            # DOWNSTREAM_COMPLETION_SPEC.md Amendment A1
IDENTITY_TOL = 1e-6                  # predictions, non-allocation scores, accounting parts (Amendment A1)
...
def final_naming(static, qualifies: dict) -> dict:
    """§2.14 from the recomputed station-arm primary-representation Booleans."""
