"""Conservative WGS84 clearance for the fixed GPW/ISO analytical units.
...
exact shoreline distance, drives exclusion. See VALIDATION_PROTOCOL.md.
"""
...
def radius_bound_km(cell_deg: float = GPW_CELL_DEG) -> float:
    ...
    WGS84 ds^2 = M^2 dphi^2 + N^2 cos^2(phi) dlambda^2; M,N <= a^2/b.
    A straight path in unwrapped latitude/longitude to any cell point therefore
    has length <= (a^2/b)*sqrt(2)*radians(cell_deg/2). Geodesic <= path length.
    """
