"""Independent reconstruction of the M2 primary result from its saved evidence (``M2_EVALUATOR_SPEC.md`` §9).

A second calculation path. It was written from the specification, the contract (with Amendment 1) and the
evaluator's artifact formats, and it shares no code with the evaluator: it imports nothing from
...
diagnostics block are not recomputed. The module was written in the same session as the evaluator, before
any M2 result existed, and is run after the result.
...
# The frozen real-data structure (contract §7); synthetic tests pass their own.
...
def verdict_conditions(delta, low, high, m49_change, worst_change, integrity, practical=PRACTICAL) -> dict:
    """Contract §5 / Amendment 1 §7; a non-finite input yields no label."""
