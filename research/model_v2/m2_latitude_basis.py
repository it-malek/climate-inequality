"""M2 latitude functional form: the frozen natural cubic basis and the H1 hemisphere interaction.

Versioned reference implementation of ``M2_DESIGN.md`` §3–§4. It is predictor-only: it builds the two
...
class DegenerateLatitudeBasis(ValueError):
    """The frozen basis cannot be built on these values. There is no fallback."""
...
def fit_state(abs_latitude_train) -> LatitudeState:
    """Knots from training latitudes only; raises on anything the frozen design cannot use."""
