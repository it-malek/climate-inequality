"""Conditional M1b robustness arms (``M1B_EVALUATION_CONTRACT.md`` §5, A2.1; ``M1B_EVALUATOR_SPEC.md`` §7).

Three registered replications, each refitting **both** models under that arm's own inputs and sample
with the exact frozen country C2 values, and each reporting both representations:

* ``m1a_area_geography``   - the frozen M1a area-consistent geography in place of station geography;
...
Execution, not merely publication, is deferred: :func:`verified_primary` refuses unless the committed,
pushed primary result verifies against its own manifest and records association support. Results are
descriptive and cannot change either verdict level. Run with
...
# ---------------------------------------------------------------------
# Authorization: a verified, committed, pushed primary result
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Arm construction: the frozen C2 values are never rebuilt or renormalized
# ---------------------------------------------------------------------
...
def m1a_geography_arm(frozen, package_dir=OUTPUT_DIR):
    """Frozen M1a area-consistent geography replaces station geography; nothing else moves."""
    ...
    ev._require(features.iso3.tolist() == frozen.table.iso3.tolist()
                ...
                'M1a feature rows do not match the frozen country order')
...
def aligned_era5_arm(frozen, package_dir=OUTPUT_DIR):
    ...
    ev._require(np.isfinite(values).all(),
                'a frozen country has no finite aligned ERA5 outcome; no missing-country rule is created')
    design = frozen.design.copy()
...
def arm_conditions(arm):
    """The same mechanical Booleans as the contract verdict, explicitly descriptive."""
...
def product_sensitivity(primary_verdict, results):
    """Contract §5: a gain confined to Berkeley is flagged product-sensitive (spec §7)."""
...
def run(primary_dir=DEFAULT_PRIMARY, out_dir=DEFAULT_OUT, *, package_dir=OUTPUT_DIR):
    ...
    (out_dir / 'm1b_conditional_provenance.json').write_text(ev.json_text({
        'contract_sha256': hydro.sha256(ROOT / ev.CONTRACT_PATH),
        'specification_sha256': hydro.sha256(ROOT / ev.SPEC_PATH),
