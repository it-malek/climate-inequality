...
Builds the M0* design and the M2 candidate design (M0* plus the two natural cubic latitude
...
same is done for the conditional M1a area-measurement arm.
...
def m1a_frame(frame: pd.DataFrame, m1a: pd.DataFrame) -> pd.DataFrame:
    ...
    if not (aligned.spatial_block.astype(str).to_numpy() == frame.spatial_block.astype(str).to_numpy()).all():
        raise ValueError('the M1a measurement file disagrees with the recorded spatial_block labels')
    out = frame.copy()
...
def encoded_identity(frame, representation, encoded, table_numerics) -> dict:
    """The full-sample M0* encoding here against the M1b encoded design, aligned by column name.

    Numeric values are taken from that design, so their identity holds by construction; the independent
    evidence is that the country-table labels rebuild every encoded dummy column exactly, that column
...
def run(out_dir=OUT) -> dict:
    ...
    if len(iso3) != 151 or len(set(iso3)) != 151 or set(iso3) != set(labels.iso3) or set(iso3) != set(m1a.iso3):
        raise ValueError('the fold artifact, country table and M1a file disagree on the 151 countries')
