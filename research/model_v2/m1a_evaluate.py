"""M1a scoring: the area-remeasured geography features against the station-based comparator.

Run ``python -m research.model_v2.m1a_evaluate`` only after ``m1a_geography`` has
produced a manifest in which every country passes every measurement gate. Otherwise
this refuses to score. Writes only ``outputs/m1a_*``.
"""
...
def load_features(design, table):
    ...
    if not manifest['all_features_pass_gates']:
        raise RuntimeError('M1a measurement gates failed; primary scoring is ineligible: '
    ...
    if any(qa.loc[code, 'gpw_cells'] != len(snapshot[code]) for code in table.iso3):
        raise ValueError('M1a support cells differ from the recorded territorial footprints')
    ...
    if features.iso3.tolist() != table.iso3.tolist() or features.Country.tolist() != design.Country.tolist():
        raise ValueError('M1a feature rows do not match the fixed country order')
...
def classify(c0, m1a, errors):
    """The pre-specified classification rules, applied mechanically to one comparator/M1a pair."""
