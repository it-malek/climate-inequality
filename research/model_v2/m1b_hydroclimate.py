"""M1b C2: baseline hydroclimatic dryness from CRU TS v4.10.

For each CRU TS v4.10 0.5 deg cell j, 1920-01..1949-12::

    P_bar_j = (1/30) sum_t PRE_{j,t}          (mm/month)
    E_bar_j = (1/30) sum_t PET_{j,t} * N_t    (mm/day x calendar days)
    x_j = log10(P_bar_j / E_bar_j)

A_{c,j} is the M1a terrestrial area. Terrestrial area in a cell that CRU's fixed land mask excludes
(a structural-mask target) takes the already-computed x of the nearest native-valid cell among its
eight neighbours (WGS84 geodesic); native values never change, harmonized values never act as
donors, and a target without such a neighbour stays unresolved::

    C2_c = sum_{native U harmonized} A_{c,j} x_j / sum_{native U harmonized} A_{c,j}

Inputs are only the M1a support record (country identifiers, terrestrial area, per-cell land area),
the GPW country grid and the pinned CRU PRE/PET files; no file holding a warming outcome is opened.
Invalid support that is not a structural-mask target stays unresolved and counts against the 0.98
coverage gate; its presence alone does not stop the build. The manifest records the SHA-256 of every
other package artifact, and ``verify_package`` checks them. Writes only ``outputs/m1b_*``. Run with
``python -m research.model_v2.m1b_hydroclimate [out_dir]``; the process exits with status 2 when any
hard stop fires.
"""
...
# Pinned CRU sources: byte sizes and SHA-256 digests of the compressed and decompressed files.
FILES = {
...
M1A_QA = OUT / 'm1a_geography_qa.csv'  # the outcome-free source of identifiers and row order
SUPPORT_COLUMNS = ['iso3', 'Country', 'terrestrial_area_km2']
GRID_ATOL_DEG = 1e-6  # ~0.1 m; GPW and CRU store exact multiples of 0.125 deg
CONTRACT_COMMIT = '60111ae1d44bdf9da6c613852a32de541f9b2d7a'  # recorded in the manifest as the rule version this build follows
...
# Fixed construction inputs, asserted before any work: the M1a support files, the GPW grid and the
# national-identifier lookup, at the digests recorded by the territorial CV, the M1a manifests and the
# product-stability audit.
FROZEN_INPUT_SHA256 = {
...
# The rule versions recorded in the manifest: one-ring structural land-mask harmonization; gate on resolved
# coverage only.
...
def check_frozen_inputs(pins=None):
    """Fail closed unless every fixed construction input matches its recorded SHA-256."""
...
def pinned_source(spec):
    ...
    if gz.stat().st_size != spec['bytes']:
        raise ValueError(f'{gz.name}: {gz.stat().st_size} bytes, the pin is {spec["bytes"]}')
    gz_hash = sha256(gz)
...
def check_support_nesting(gpw_lat, gpw_lon):
    """Assert each GPW 0.25 deg cell lies inside CRU 0.5 deg cell (row // 2, col // 2).
...
def window_months(times):
    """Indices of exactly the 360 window months, and their calendar day counts."""
    ...
    if len(selected) != N_MONTHS or not (selected == expected).all():
        raise ValueError('time axis does not contain exactly the 360 window months')
...
def cell_climatology(pre, pet, days):
    """Per-cell P_bar, E_bar, x and validity for arrays shaped (360 months, lat, lon)."""
...
def country_cells(codes, land_area, country_of_cell):
    """Long table of (country index, CRU row, CRU col, terrestrial km^2) from GPW 0.25 deg cells."""
...
def support_quantities(stn, valid):
    """Per-cell PRE station support over the window, audited over valid cells.
...
def _neighbour_distance_table():
    """WGS84 geodesic distance (km) from a row-i CRU centre to its neighbour at (di, |dk|); shape (360, 3, 2).

    Karney's inverse geodesic (pyproj ``Geod``) on the WGS84 ellipsoid of the territorial CV,
...
def variable_state(record_fill, window_fill, window_finite, window_mean, n_record_months, n_window_months=N_MONTHS):
    """Per-cell state of one variable, checked in this order.
...
def one_ring_donors(targets, native, x):
    """Nearest native-valid donor among the eight cells adjacent to each target.
...
def aggregate(resolved, p_bar, e_bar, supported, mean_stn, pure, n):
    """Per-country C2, area partition, donor-distance QA and the native-cell station-support audit."""
...
def harmonization_summary(resolved, support):
    """Global harmonization counts, areas and donor distances (distinct cells unless stated)."""
...
def hard_stops(qa, codes):
    """The hard-stop rules, one record per country and rule.

    The mere presence of non-structural invalid support does not stop the build: that area stays
...
def build(out=OUT):
    ...
    code_hash = sha256(Path(__file__))  # taken before any work, so the manifest identifies the code that produced it
    check_frozen_inputs()
    ...
    if not np.array_equal(native, (pre_state == COMPLETE) & (pet_state == COMPLETE)):
        raise AssertionError('variable states do not reproduce the native validity rule')
