"""Legacy 1 deg cell-centre distances: not a territorial clearance guarantee.

The primary cross-validation protocol uses ``territory.py``. This module is retained
only to reproduce the earlier diagnostic and candidate-design artifacts, which are
not overwritten.
...
is therefore defined on the **minimum great-circle distance between any 1 deg land
...
def border_distance_matrix(iso_order: list[str]) -> pd.DataFrame:
    ...
    for code in iso_order:
        m = iso3 == code
        if not m.any():
            raise ValueError(f"no 1 deg cell assigned to {code}")
        points[code] = xyz[m]
