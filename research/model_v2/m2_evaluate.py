"""M2 scoring: natural cubic latitude terms and a hemisphere interaction added to the M0* design.
...
* Numeric predictors are the exact fixed scoring values (``M0star`` rows of the M1b encoded design); the
  country table supplies identifiers, M49 labels, station coordinates and categorical labels only.
* The latitude state of every fit comes from that fit's training rows (``m2_latitude_basis``).
* Before any M2 fit, every fit's state, ranks and held-out extrapolation are checked against the committed
  predictor-only feasibility record, and both M0* representations must reproduce their reference cards.
...
# Metrics on which M0* must reproduce its recorded reference card (the M1b list, unchanged).
REFERENCE_METRICS = (
...
# ---------------------------------------------------------------------
# Scoring-input gate and the fixed frame
# ---------------------------------------------------------------------
...
def verify_pins() -> dict:
    ...
    for rel, (pin, commit) in ALL_PINS.items():
        ...
        _refuse(digest == pin, f'{rel}: sha256 {digest} differs from the record fixed at {commit} ({pin})')
...
class Frozen:
    ...
    encoded: pd.DataFrame   # the M1b encoded design
    identity: dict
...
def numeric_frame(encoded, representation, iso3):
    ...
    _refuse(set(iso3) <= set(wide.index), f'{representation}: the encoded design does not cover the 151 countries')
    frame = wide.loc[list(iso3), columns].reset_index()
    _refuse(np.isfinite(frame[columns].to_numpy(float)).all(), f'{representation}: non-finite encoded numerics')
    return frame
...
def encoded_identity(frame, representation, encoded):
    """Full-sample M0* encoding against the encoded design, aligned by column name."""
    ...
    _refuse(np.array_equal(values[:, dummies], matrix[:, dummies]),
            f'{representation}: dummy columns rebuilt from the labels differ from the encoded design')
    _refuse(np.array_equal(values, matrix), f'{representation}: rebuilt design differs from the encoded design')
...
def outcome_vector(table):
    ...
    y = (frozen[frozen.variant == 'drop_per_capita'].set_index('iso3').loc[table.iso3, 'observed']
         .to_numpy(float))
    _refuse(np.isfinite(y).all(), 'non-finite outcome')
    ...
    _refuse(np.array_equal(design[OUTCOME_COL].to_numpy(float), y),
            'the recorded ordered outcome differs from the pipeline outcome')
    ...
    _refuse(np.array_equal(m1b.set_index('iso3').loc[table.iso3, 'observed'].to_numpy(float), y),
            'the recorded ordered outcome differs from the committed M1b prediction record')
    return y
...
def fold_identities(table, distance, folds):
    ...
    _refuse(np.array_equal(derived, membership), 'primary memberships differ from the corrected recorded memberships')
    ...
    for protocol, frozen_column, m1b_column in (('m49_subregion_lo', 'm49_subregion_lo', 'm49_fold'),
        ...
        _refuse(np.array_equal(ids, frozen_membership[frozen_column].to_numpy())
                ...
                f'{protocol} fold ids differ from the recorded definitions')
...
def build_frozen() -> Frozen:
    """Load the fixed 151-country scoring frames and check every identity."""
    table = pd.read_csv(ROOT / COUNTRY_TABLE, usecols=TABLE_COLUMNS, float_precision='round_trip')[TABLE_COLUMNS]
    _refuse(len(table) == 151 and table.iso3.is_unique, 'the country table is not the fixed 151 countries')
...
def run_protocol(model, frame, frozen, protocol, outcome=OUTCOME_COL):
    """Out-of-fold predictions through ``cv.cross_validate`` with a record of every fit."""
...
def predictor_only_states(frozen, frame, representation):
    """Every fit's latitude state, ranks and extrapolation, without any outcome."""
    rows = []
...
def reference_check(card, reference, label, metrics=REFERENCE_METRICS, atol=REFERENCE_ATOL):
    ...
    for metric in metrics:
        ...
        _refuse(abs(got - want) <= atol, f'{label} does not reproduce the recorded reference card: {metric} {got!r} vs {want!r}')
...
def verdict(comparator, candidate, paired, integrity_ok):
    """The pre-specified verdict rules, applied mechanically; non-finite inputs are refusals, never ``False``."""
...
def score_comparators(frozen, frames, reference_cards, outcome=OUTCOME_COL):
    """Both M0* representations, checked against their reference cards before any M2 fit."""
    arms = {}
    for representation in REPRESENTATIONS:
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='Score M2 under the pre-specified protocol.')
