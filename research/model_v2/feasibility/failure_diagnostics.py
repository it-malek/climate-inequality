"""Per-pixel diagnostics for the four countries that failed the original M1a coverage gates.

Read-only: prints a JSON summary and writes nothing to the repository."""
import json
import numpy as np
import pandas as pd
import xarray as xr
...
for code in FAIL:
    ...
    # concentration of missing area over 1 deg cells
    key = (r // 60) * 360 + c // 60
    ...
    if uncl.any():
        # nearest classified group among the 8 neighbouring 0.5 deg pixels, for context only
