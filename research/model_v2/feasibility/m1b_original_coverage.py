"""M1b feasibility audit under the original coverage rule (native-valid cells only, no harmonization).

Read-only characterisation of the original C2 coverage failure, plus a byte comparison of two
independent rebuilds against the preserved outputs. Inputs are only country identifiers, the pinned
...
def grid_alignment():
    """GPW 0.25 deg cells nest exactly 2 x 2 inside CRU 0.5 deg cells after the ascending-latitude flip."""
