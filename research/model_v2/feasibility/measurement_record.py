"""Pre-scoring M1a measurement record under Amendment 1 (writes M1A_MEASUREMENT_RECORD.md and a CSV)."""
import json
...
L = ['# M1a measurement record under Amendment 1 — before scoring', '',
     '**Recorded before `m1a_evaluate.py` was run.** Builder `m1a_geography.py` sha256 '
     f"`{manifest['code_sha256']}` (commit `10e117b`).",
     '',
     '## Change control',
     '',
     ...
     '* **No changes after observing the builds.** After the amended builds were observed, no parameter, source, threshold, '
     'country membership, measurement rule, evaluator code or evaluation contract changed. `git diff 10e117b` is empty for '
     'the builder, specification, evaluator and contract.',
     '',
     '## Consistency with the feasibility build', '']
L += [f'* {k.replace("_", " ")}: **{v}**' for k, v in checks.items()]
L += ['', '## Köppen classes: effect of the identifiability rule', '',
...
L += [f'* {r.iso3}: original rule → {r.original_rule if isinstance(r.original_rule, str) else "missing (failed the 95% classified gate)"}; '
      f'amended rule → {r.amended_rule}' for r in changed_k.itertuples()] or ['* none']
L += ['', 'No other country changed class. The change arises only from the precommitted identifiability rule.',
      f'For context, and not as an effect of the amendment: {int(qa.climate_zone_changed.sum())} countries\' area-dominant class differs from '
...
L += ['', '## Sensitivities S1–S4 (Amendment 1), difference from the primary elevation (m)', '',
...
L += ['', f"S3 (ring-2) coverage ranges from {table.elevation_s3_coverage.min():.4f} to {table.elevation_s3_coverage.max():.4f}. "
      f"Total unresolved area is {table.elevation_unresolved_area_km2.sum():,.0f} km² "
...
Path('research/model_v2/M1A_MEASUREMENT_RECORD.md').write_text('\n'.join(L) + '\n')
print(json.dumps(checks, indent=1))
print(changed_k.to_string(index=False))
print('\n'.join(L[L.index('## Elevation: pre/post distribution (amended − original centre-only, metres)'):L.index('## Full per-country record')]))
