"""Record of the stopped Amendment 1 C2 build (hard stop A1.7: non-structural invalid support cell).

Read-only. Compares two independent amended builds byte-for-byte, checks exact invariants against the
preserved original-rule build (c5ff9b5), summarises coverage and harmonization QA, and reads the raw
CRU values of every non-structural support cell. Reads only C2 build outputs and the pinned CRU files;
...
def main(build_b):
    ...
    if not all(r['byte_identical'] for r in files.values()):
        raise AssertionError(f'amended builds differ: {files}')
