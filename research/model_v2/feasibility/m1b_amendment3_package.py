"""Verify two independent C2 builds under the resolved-coverage gate and install the predictor package.

Runs before any outcome is read. Reads only C2 build outputs, the preserved evidence directories and
Git; no warming outcome, residual or score is read and no C2 value is computed. Fails closed on any
difference.
...
2. Checks exact invariance against the preserved stopped build and the original-rule build.
...
# Manifest keys allowed to differ from the stopped build; the stopped build's own harmonization-note key
# (replaced by 'amendments') is added in main() from that manifest.
DECLARED_MANIFEST_CHANGES = {'amendments', 'gate_version', 'git', 'contract_sha256', 'support_inputs',
...
def main(build_commit, build_a, build_b, install=True):
    ...
    countries = pd.read_csv(h.M1A_QA, usecols=['iso3', 'Country'])
    ...
    population = {'ordered_151_countries_equal_frozen_record': features.iso3.tolist() == countries.iso3.tolist()
                  and features.Country.tolist() == countries.Country.tolist() and len(features) == 151}
    ...
    note_keys = {k for k in stopped_manifest if k not in manifest and k.startswith('amend')}
    stopped['manifest_changes_within_declared_set'] = changed <= DECLARED_MANIFEST_CHANGES | note_keys
