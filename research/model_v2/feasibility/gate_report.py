"""Generate the per-country coverage tables of the original M1a gates as Markdown (uncommitted scratch output)."""
...
L=['# M1a measurement gate report (original gates)', '',
   '**No M1a model was fitted or scored under these gates.**',
   ...
   '## Complete 151 x 6 coverage table', '',
   'Coverage = valid terrestrial support / terrestrial support. Required: latitude and hemisphere 1.00 (geometry); elevation and continentality >= 0.98; climate >= 0.95 classified **and** winner margin > unclassified area; spatial block nonmissing. FAIL marks a failed gate.', '',
   '| ISO3 | Country | Terrestrial km^2 | abs_latitude | hemisphere | elevation | continentality | climate_zone (classified) | Koppen margin/unclassified | spatial_block |',
   '|---|---|---:|---:|---:|---:|---:|---:|---:|---:|']
for iso in wide.index:
    ...
    def cell(f, w=w, p=p):
        return '{:.4f}'.format(w[f]) + ('' if p[f] else ' FAIL')
...
L.append('')
L.append('Classified Koppen coverage in [0.95, 0.96): ' + (', '.join(f'{r.iso3} {r.achieved:.4f}' for r in near_k.itertuples()) or 'none'))
L.append('')
L.append('Koppen winner margin < 3 x unclassified area (passes, but least robust): ' + (', '.join(f'{i} margin {r.koppen_winner_margin_km2:,.0f} km^2 vs unclassified {r.koppen_unclassified_km2:,.0f} km^2' for i, r in marg.iterrows()) or 'none'))
