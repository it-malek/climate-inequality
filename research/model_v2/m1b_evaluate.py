"""M1b scoring: baseline hydroclimatic dryness added to the M0* design.

M1b = M0* + ``baseline_dryness`` in a research-only ``hydroclimate`` group, reported in both the
primary total-CO2 representation and, unconditionally, the per-capita representation. Only the
primary pair determines the verdict.
...
its redundancy record and every fixed scoring input match their recorded digests, before any target
...
# Pre-specified verdict thresholds. None is recomputed, relaxed or made outcome-dependent here.
...
# Pre-specified reproduction and equivalence tolerances.
REFERENCE_ATOL = 1e-10      # rank-audit reference-card reproduction
...
# Fixed scoring inputs: SHA-256 and the commit that recorded each file. Verified before any fit.
...
CODE_PATH = (
    ...
    'research/model_v2/evidence/M1B_EVALUATOR_SPEC.md', 'research/model_v2/evidence/M1B_EVALUATION_CONTRACT.md',
...
CONTRACT_PATH = 'research/model_v2/evidence/M1B_EVALUATION_CONTRACT.md'
SPEC_PATH = 'research/model_v2/evidence/M1B_EVALUATOR_SPEC.md'
...
# Metrics on which a comparator must reproduce its recorded reference card.
REFERENCE_METRICS = (
...
class M1bDesign(cv.V1Design):
    """The V1 estimator under the research schema extension (M0* when C2 is absent)."""
...
def require_hash_seed():
    if os.environ.get('PYTHONHASHSEED') != '0':
        raise RuntimeError('scoring requires PYTHONHASHSEED=0 for byte-reproducible output')
...
def _require_manifest_fields(manifest):
    """Explicit required-field and type validation of the measurement manifest."""
    ...
    _require(manifest['contract_freeze_commit'] == hydro.CONTRACT_COMMIT, 'rule-version commit')
...
def _require_record_fields(record, manifest, manifest_sha256):
    """Explicit required-field validation of the redundancy record."""
...
def verify_scoring_inputs(out_dir=OUTPUT_DIR):
    """Fail closed before any target column is loaded."""
    ...
    for rel, (pin, commit) in FROZEN_SCORING_INPUTS.items():
        ...
        _require(digest == pin,
                 f'{rel}: sha256 {digest} differs from the record fixed at {commit} ({pin})')
...
def build_frozen(inputs):
    """Load the fixed 151-country scoring frame and check every identity."""
    ...
    _require(features.iso3.tolist() == support.iso3.tolist()
             ...
             'C2 row order differs from the M1a support record')
    ...
    _require(design.Country.tolist() == features.Country.tolist() == table.Country.tolist(),
             'the scored design is not the fixed country population in the fixed order')
    ...
    _require(np.array_equal(y, observed),
             'the Berkeley outcome differs from the fixed ordered outcome record')
    ...
    _require(labels.m49_subregion.notna().all()
             ...
             'M49 labels differ from the recorded definition')
    ...
    _require(membership.iso3.tolist() == table.iso3.tolist()
             ...
             'recorded fold-membership labels')
    ...
    _require(np.array_equal(folds['m49_subregion_lo'][0], membership.m49_subregion_lo.to_numpy()),
             'M49 holdout construction differs from the recorded definition')
    _require(np.array_equal(folds['random10'][0], membership.random_10fold.to_numpy()),
             'the random 10-fold seed-0 reference differs from the recorded definition')
    ...
    _require(np.array_equal(derived, frozen_membership),
             'primary fold memberships differ from the corrected recorded memberships')
    ...
    _require(n_train.iso3.tolist() == table.iso3.tolist()
             ...
             'primary training counts differ from the recorded values')
...
def protocol_card(frame, table, result, station_w):
    """The fixed metric set for one protocol. No paired field lives in a card."""
...
def reference_check(card, reference, label, metrics=REFERENCE_METRICS, atol=REFERENCE_ATOL):
    """Reproduce a recorded reference card to ``atol``; every compared value must be finite."""
    rows = {}
    for metric in metrics:
        ...
        _require(abs(got - want) <= atol,
                 f'{label} does not reproduce the recorded reference card: {metric} {got!r} vs {want!r}')
...
def verdict(comparator, candidate, paired, sign):
    """The pre-specified verdict rules, applied mechanically. Non-finite inputs are a refusal, never a ``False``."""
...
def diagnostics(comparator, candidate, paired):
    """Descriptive diagnostics and the three-dimension summary (never verdict conditions)."""
    delta = float(paired['delta_rmse'])
    lo, hi = (float(v) for v in paired['country_bootstrap_95_interval'])
...
def equivalence(arms):
    """The two representations span the same column space, so predictions must agree."""
...
def score_representations(frozen, *, reference_cards, expected_columns=EXPECTED_COLUMNS):
    ...
    Both comparators are scored and checked against their recorded reference cards **before** either
    candidate is fitted. ``expected_columns`` is ``None`` for an arm whose categorical
...
def support_context(package_dir=OUTPUT_DIR):
    """Station-support provenance QA, reported alongside the result (context, never a condition)."""
...
def non_deciding_conditions(entry):
    """A non-deciding representation reports the same Booleans without a verdict label.

    Exactly one verdict is reported, and acceptance rests on the primary total-CO2 representation
    alone, so the per-capita arm carries a descriptive label, exactly as a conditional robustness
    arm does.
    """
...
def redundancy_context_from(record):
    """Redundancy context, taken from the verified redundancy record rather than transcribed by hand."""
...
def evaluate(frozen, out_dir, *, provenance, reference_cards, support_qa=None,
    ...
    """Score both representations, apply the pre-specified verdict rules and write the evidence.

    The output root is created only once the comparators have reproduced their reference cards and both
    ...
    result = {
        ...
        'representations': {
            representation: {
                ...
                # Exactly one verdict is reported: the primary representation's. The per-capita
                # arm carries the same Booleans under a descriptive label.
...
def reproducibility_record(run_a, run_b, *, code=None):
    """Byte-compare two unchanged-code runs' deterministic artifacts.
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='Score M1b under the pre-specified protocol.')
    ...
    provenance = {
        'contract_sha256': prov.sha256(ROOT / CONTRACT_PATH),
        'specification_sha256': prov.sha256(ROOT / SPEC_PATH),
...
def cli_exit_code(outcome):
    """Process exit status for a :func:`main` outcome.

    Scoring returns a result carrying ``integrity_passed`` (0 when it holds, else 3); ``--compare`` returns a reproducibility record carrying ``byte_identical`` (0 when
