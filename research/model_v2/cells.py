"""What the country outcome discards: within-country structure of the 1 deg trend field.

The V1 primary outcome collapses every 1 deg Berkeley Earth cell trend inside a country
...
def load_cell_field(path: Path = CELL_TRENDS_PATH) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """``(lats, lons, slopes, iso3)`` of the cached 1 deg field (``iso3 == ""`` off-country)."""
