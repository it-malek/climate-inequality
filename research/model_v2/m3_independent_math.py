...
A verification path for the M3 spatial rules, written without reading or importing the M3 estimator
module. Given saved theta, beta, neighbour lists and designs, it rebuilds the
...
Graphs. For a node set N in canonical order, node i's kNN8 neighbours are the first 8 of N \\ {i}
...
Likelihood. With M(theta) = I - theta W and w_k the complex eigenvalues of W,
...
``lstsq(rcond=None)`` cut-off. ``maximize`` re-runs the fixed optimizer (199-point grid, first maximal
index, golden section with exactly 60 iterations, earliest-evaluation ties, |theta| <= 0.99 - 1e-6).

Fitted quantities. Spatial lags are explicit loops over neighbour lists, never products with
...
rule is stated as the set {0} U {beta}; ``cv.V1Design.predict``, which is applied exactly, averages the
array, so equal effects are not merged here.)

Accounting. Coalition R2 is a projection R2. Rank deficiency is handled by greedy column trimming:
...
Expected information. The Anselin (1988) entries are assembled from explicit element sums
...
class StructuralFailure(ValueError):
    """A graph cannot be built under the graph rules. There is no fallback."""


class EvaluationFailure(ValueError):
    """One likelihood evaluation failed a likelihood condition."""
...
class Profile:
    """One concentrated-likelihood evaluation; ``failure`` names the first likelihood condition that failed."""
...
def profile(family: str, y, x, w, theta: float, eigenvalues=None) -> Profile:
    """l_c(theta), beta(theta), sigma2(theta) through eigenvalues and QR, with the failure rules."""
...
def maximize(objective: Callable[[float], float]) -> Maximization:
    """The fixed search on any objective; the objective raises EvaluationFailure to fail a point."""
...
def reference_fit(family: str, y, x, w) -> ReferenceFit:
    """The whole fit re-derived on the eigenvalue/QR likelihood."""
    family = _family(family)
    y, x, w = _regression(y, x, w)
