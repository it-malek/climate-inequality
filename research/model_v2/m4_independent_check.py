"""Independent reconstruction of the M4 final assessment from its saved evidence (``M4_EVALUATOR_SPEC.md`` §4).

A second calculation path, written from ``DOWNSTREAM_COMPLETION_SPEC.md`` §3, ``M4_EVALUATOR_SPEC.md``,
``M4_EVIDENCE_INVENTORY.md`` and the evaluator's artifact formats. It imports nothing from ``m4_evaluate``,
...
1e-12. M0* sensitivity anchor: 1e-9 (spec §3.4). Labels, counts, flags, draw rows, identifiers, naming fields
...
* the M0* anchor is required for every sensitivity cell in either branch; a non-computable M0* cell fails it
  (spec §2 item 2: refusal otherwise);
...
anchor against the M2 conditional arm is not re-checked. Written in the same session as the M4 evaluator,
before any M3 or M4 result existed; executed after the result.
...
# The frozen real-data structure; synthetic tests pass their own.
...
class DegenerateDraw(ValueError):
    """A bootstrap draw on which the frozen design cannot be built or the shares are not finite."""
