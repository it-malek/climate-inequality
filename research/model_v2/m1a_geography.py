...
No model is fitted here and nothing outside ``research/model_v2/outputs/m1a_*`` is
written. Run with ``python -m research.model_v2.m1a_geography``.

Support ``D_c`` is the union of the country's GPW v4 rev11 band-11 0.25 deg cells
intersected with GSHHG 2.3.7 full-resolution land, (L1 - L2) U (L3 - L4). The
ETOPO 2022 60 arc-second pixel lattice is the quadrature grid. Its pixel edges fall
exactly on the 0.25 deg GPW and 0.5 deg Koppen edges, so every pixel lies in exactly
one GPW cell and one Koppen pixel. Pixel land fractions come from exact polygon
clipping in longitude/latitude, scaled by the pixel's spherical area
``R^2 dlambda (sin phi2 - sin phi1)``.
"""
...
CELL_PIX = 15                 # ETOPO pixels per 0.25 deg GPW cell
KOPPEN_PIX = 30               # ETOPO pixels per 0.5 deg Koppen pixel
N_ROWS, N_COLS = 180 * PIX, 360 * PIX
BLOCK_CELLS = 64              # initial recursion block, 16 deg
...
COMPLETION_RING = 1            # coastal completion: queen-adjacent centre-resolved pixels of the same unit
SENSITIVITY_RING = 2
DISTANCE_TOL = {'absolute_km': 0.1, 'relative': 0.001}
DISTANCE_BLOCKS = (3, 5)       # pixels per side: 3 arc-minute primary quadrature, 5 arc-minute convergence check
...
def coastline_segments(land):
    """Boundary segments of the land geometry without artificial seam edges.

    Edges lying on the +-180 deg meridian or the south-pole cap are polygon cuts, not
...
def land_geometry(levels):
    """(L1 - L2) U (L3 - L4) for already clipped level polygons."""
    def union(level):
...
@dataclass
class Records:
    """60 arc-second pixel quadrature records (a GPW cell's pixels are always emitted together)."""
...
def cell_records(geom, r0, c0, n=CELL_PIX, records=None):
    """Clip an n x n block of 60 arc-second pixels (lower-left pixel r0, c0) against land geometry."""
...
class Sums:
    ...
    def __post_init__(self):
        ...
        self.dist60s = [z(), z()]  # exact 60 arc-second pixel quadrature, DISTANCE_60S_CHECK countries only
        ...
        # Coastal completion: country index + 1 of centre-resolved pixels (lazy zero pages),
...
def accumulate(records, sums, country_of_cell, elevation, koppen_group, coast, area_rows, check_idx=()):
    ...
    # Distance quadrature: one evaluation per land-area centroid of each b x b pixel block.
...
def dominant_climate(areas):
    """Area-dominant A-E, identified only if A1 > A2 + U (areas: A..E, unclassified)."""
...
def finalize_elevation(sums, elevation):
    """Coastal support completion and its sensitivities, after every pixel is known."""
    n = sums.n
...
def build(out=OUT, batch_pixels=6_000_000):
    ...
    # Deterministic refinement: any country whose 3 arc-minute and 5 arc-minute distance quadratures
    # disagree beyond tolerance, and has no 60 arc-second value yet, gets an exact 60 arc-second pixel pass.
