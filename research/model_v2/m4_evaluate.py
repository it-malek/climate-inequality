"""M4 final assessment: share uncertainty, buffer sensitivities, product check and the final record.

M4 changes no model. It computes only the evidence the pre-specified inventory marks as missing, for the retained
...
LEGACY_SUMMARY_SHA256 = None   # pinned below through PINS
...
# ---------------------------------------------------------------------
# C. Product check of the eventual model
# ---------------------------------------------------------------------
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='M4 final assessment under the pre-specified protocol.')
    ...
    if args.compare:
        record = {
            ...
            'run_a': args.compare[0], 'run_b': args.compare[1],
