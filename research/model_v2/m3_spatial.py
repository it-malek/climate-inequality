...
Versioned reference implementation of ``DOWNSTREAM_COMPLETION_SPEC.md`` §2.2-§2.7 and §2.11, NumPy
only (no spatial-econometrics library). Every numerical rule is frozen: graph and tie rule, grid,
...
Graphs (§2.2). Nodes are canonical row indices of one full distance matrix D. kNN8 on a canonical-ordered
...
Models (§2.3-§2.4), with M(theta) = I - theta W and W row-standardised:

    SEM   y = X beta + u,   u = lambda W u + eps          SAR   y = rho W y + X beta + eps

Concentrated log-likelihood (§2.5). SEM regresses M y on M X, SAR regresses M y on X, both with
...
Accounting (§2.11). A_static = 1 - RSS_S/TSS, A_dependence = (RSS_S - RSS_eps)/TSS,
...
class SpatialFitFailure(ValueError):
    """A structural failure of a frozen M3 fit (§2.8): the fit is non-computable, never retried."""
...
class IntegrityFailure(RuntimeError):
    """An arithmetic identity of §2.11 failed: an integrity failure (§2.8), the line stops."""
...
# --- concentrated likelihood and its maximization (§2.5) --------------------------------------
...
def concentrated_loglik(family: str, theta: float, y, x, w) -> Evaluation:
    """lc(theta) with beta-hat(theta) and sigma2-hat(theta); raises on every frozen failure condition."""
    _family(family)
    y, x, w = _data(y, x, w)
    n, p = x.shape
...
def fit_spatial(family: str, y, x, w, columns: Sequence[str] | None = None) -> SpatialFit:
    """Gaussian ML fit of SEM or SAR on one training graph (§2.5); raises SpatialFitFailure."""
    _family(family)
    y, x, w = _data(y, x, w)
    validate_graph(w)
...
# --- fitted values, residuals, innovations (§2.6) --------------------------------------------
...
# --- model-based parameter uncertainty (§2.5) --------------------------------------------------
