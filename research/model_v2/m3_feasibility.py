...
This checks that the M3 spatial estimator is executable on the fixed
sample, for both possible retained static specifications (M0* and M2), without reading any warming outcome:

* **Graphs** for both weights arms (station centroid, land centroid) and every fit the protocol requires or
  ...
  finiteness of log|I - theta W| over the whole fixed grid, and the spectrum's extreme values.
* **Estimator executability** on the real encoded designs with a **synthetic** outcome drawn from each family
  ...
  both families and both static designs, through the fixed maximizer; failures, interior status, local
...
def synthetic_fits(frame, dist, protocol_plans):
    """Executability of the estimator on real designs with a synthetic outcome (no warming data)."""
    rows = []
...
def synthetic_shrinkage(frame, dist, draws=100, thetas=(0.0, 0.4)):
    ...
    designs and two true dependence values, recording before any result how far the static designs pull
...
def run(out_dir=OUT):
    ...
    if digests['station'] != identity['station_weights_sha256'] or digests['land'] != identity['area_weights_sha256']:
        raise ValueError('full-sample kNN8 weights differ from the recorded M1b identities')
    ...
    for (label, train, _test), row in zip(protocol_plans['primary_loco'], folds.itertuples()):
        if label != row.iso3 or sorted(table.iso3.iloc[train]) != sorted(row.primary_training_iso3.split('|')):
            raise ValueError(f'primary membership of {row.iso3} differs from the fold artifact')
