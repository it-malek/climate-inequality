"""Independent re-derivation of the M1b scorecard from its own saved evidence.

This module shares no code with the evaluator it checks. It re-implements, from the
evaluation rules alone, every reported quantity that the saved per-country evidence
determines, and compares each one with the saved scorecard:
...
3. the paired resampling interval, re-implemented here: 2,000 resamples,
...
4. the sign condition and verdict label, rebuilt from the saved
...
def paired_interval(comparator_error: np.ndarray, candidate_error: np.ndarray) -> dict[str, float]:
    """Paired country resampling of fixed out-of-fold errors, no refitting."""
...
def check_verdict(
    ...
    """Sign condition, S1/S2, A1-A5 and the verdict label, rebuilt from the pre-specified rules."""
    records: list[dict[str, Any]] = []
    for representation in REPRESENTATIONS:
