"""M3 scoring under ``DOWNSTREAM_COMPLETION_SPEC.md`` §2 and ``M3_EVALUATOR_SPEC.md``.
...
any estimate, lag or prediction. Accounting is the frozen multi-part identity with filtered-trend group shares.
...
PREDICTION_ATOL = 1e-6    # spatial representation identity (DOWNSTREAM_COMPLETION_SPEC.md Amendment A1)
THETA_ATOL = 1e-5
...
def reproduce_static(frozen, models, primary=M2_PRIMARY) -> dict:
    """Refit the static models with the frozen M2 functions; predictions and cards must match to 1e-10."""
...
def arm_distance(frozen, weights):
    ...
    if prov.array_digest(knn_weights(dist, 8)) != expected:
        raise prov.ExecutionRefused(f'the {weights} kNN8 matrix differs from the frozen identity')
    return dist
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='Score the M3 spatial families under the frozen specification.')
