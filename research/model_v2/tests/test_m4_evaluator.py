...
def scoring():
    return synthetic_scoring(n=60, seed=5)


def test_country_draws_are_the_v1_resampling_sequence(scoring):
...
def test_block_draws_are_the_v1_resampling_sequence(scoring):
    frame = scoring.frames['primary_total_co2']
...
def test_bootstrap_draws_record_every_draw_and_account(scoring, monkeypatch, model):
...
def test_a_degenerate_draw_is_counted_not_replaced(scoring, monkeypatch):
    monkeypatch.setattr(m4, 'N_BOOT', 3)
    frame = scoring.frames['primary_total_co2'].assign(abs_latitude=5.0)
...
def test_sensitivity_protocols_and_distances(scoring):
    territory = m4.sensitivity_frozen(scoring, 'buffer_territory_1000km')
    assert np.array_equal(territory.distance, scoring.distance)
...
def test_a_group_without_columns_in_a_draw_keeps_a_zero_share(scoring):
    frame = scoring.frames['primary_total_co2'].assign(income_group='High-income countries')
...
def test_compare_uses_only_m4_artifacts(tmp_path, monkeypatch, changed):
    """M4 comparison must not try reading M3 files before comparing its own."""
    a, b = tmp_path / "a", tmp_path / "b"
    a.mkdir()
    b.mkdir()
    names = (*m4.DETERMINISTIC_ARTIFACTS, m4.RESULT_MANIFEST)
    for name in names:
        (a / name).write_text("same")
        (b / name).write_text("same")
    if changed:
        (b / names[0]).write_text("different")
    # Run metadata must neither be required nor compared.
    (a / m4.RUN_METADATA).write_text("run A")
    monkeypatch.setattr(prov, "frozen_code_state", lambda files: {"commit": "test"})
    record = m4.main(["--compare", str(a), str(b)])
    assert set(record["artifacts"]) == set(names)
    assert record["byte_identical"] is (not changed)
    assert record["excluded_as_run_metadata"] == [m4.RUN_METADATA]
