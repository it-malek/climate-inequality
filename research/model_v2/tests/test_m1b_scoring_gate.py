...
is fitted: these tests exercise the refusals of the input gate.
"""
import hashlib
import io
import json
...
COUNTRIES = pd.read_csv(red.SUPPORT_RECORD, usecols=['iso3', 'Country'])
# Identity fields of the accepted measurement package, copied from its committed manifest.
ACCEPTED_MANIFEST = json.loads((hydro.OUT / hydro.MANIFEST).read_text(encoding='utf-8'))
...
def write_package(out, c2=None, manifest_overrides=None, record_overrides=None,
    ...
    values = np.linspace(-1.0, 0.5, len(COUNTRIES)) if c2 is None else np.asarray(c2, dtype=float)
    features = COUNTRIES.assign(baseline_dryness=values)
    ...
    country_list = sha(('\n'.join(features.iso3) + '\n').encode())
    manifest = {
        'contract_freeze_commit': ACCEPTED_MANIFEST['contract_freeze_commit'],
        'amendments': ACCEPTED_MANIFEST['amendments'], 'gate_version': hydro.GATE_VERSION,
        'git': {'commit': ACCEPTED_MANIFEST['git']['commit'], 'code_path_matches_commit': True},
...
def test_an_internally_consistent_but_unaccepted_package_is_rejected(tmp_path):
...
def test_a_consistent_accepted_package_passes_the_gate(tmp_path, monkeypatch):
    ...
    pin, label = ev.FROZEN_SCORING_INPUTS[ev.COUNTRY_TABLE]
    assert inputs.digests['frozen_scoring_inputs'][ev.COUNTRY_TABLE] == {'sha256': pin, 'frozen_at': label}
...
def test_missing_malformed_or_wrong_identity_manifest_fields_are_rejected(mutate):
    manifest = {
        'contract_freeze_commit': ACCEPTED_MANIFEST['contract_freeze_commit'],
        'amendments': ACCEPTED_MANIFEST['amendments'],
...
@pytest.mark.parametrize('override', [
    ...
    {'measurement_git_commit': '0' * 40}, {'measurement_gate_version': 'stale'},
...
@needs_inputs
def test_the_real_package_and_pinned_inputs_build_the_scoring_frame():
    inputs = ev.verify_scoring_inputs()
    scoring = ev.build_frozen(inputs)
    assert scoring.n == 151 and scoring.design.Country.tolist() == scoring.table.Country.tolist()
    assert scoring.identity['outcome_source_column'] == 'trend_c_per_decade_area_weighted'
    assert scoring.identity['representation_identity_max_abs_error'] <= ev.IDENTITY_ATOL
    assert scoring.identity['c2_vector_sha256'] == inputs.record['provenance']['c2_vector_sha256']
    assert set(scoring.folds) == {ev.PRIMARY, 'm49_subregion_lo', 'random10'}
...
def test_a_changed_outcome_vector_is_rejected(monkeypatch):
    ...
    with pytest.raises(ValueError, match='ordered outcome record'):
        ev.build_frozen(inputs)


@needs_inputs
def test_a_changed_pinned_scoring_input_is_rejected(monkeypatch):
    pinned = dict(ev.FROZEN_SCORING_INPUTS)
    pinned[ev.RANK_AUDIT] = ('0' * 64, 'synthetic')
...
@needs_inputs
def test_the_comparators_reproduce_the_committed_rank_audit_cards():
    """M0* must reproduce its committed rank-audit card. No C2 is added, so nothing is fitted to it."""
    inputs = ev.verify_scoring_inputs()
    scoring = ev.build_frozen(inputs)
    cards = json.loads((ROOT / ev.RANK_AUDIT).read_text())['cards']
    for representation, spec in ev.REPRESENTATIONS.items():
        comparator = scoring.design.drop(columns=spec['drop']).reset_index(drop=True)
        assert ev.FEATURE not in comparator.columns
        scored = ev.score_model(comparator, scoring,
...
def test_a_candidate_design_on_real_predictors_is_full_rank_at_twenty_one_columns():
    ...
    inputs = ev.verify_scoring_inputs()
    scoring = ev.build_frozen(inputs)
    placeholder = np.linspace(-1.0, 1.0, scoring.n)
    frame = scoring.design.drop(columns='cum_co2_per_capita').reset_index(drop=True)
