"""Conditional M1b robustness arms: verification of the primary result, arm construction and reporting.

The refusals below are exercised without scoring any arm. Scored fixtures are synthetic except where a
test explicitly uses a committed artifact.
"""
import dataclasses
import json
import subprocess
...
from research.model_v2.tests.test_m1b_evaluator import synthetic_scoring

REAL_INPUTS = DEFAULT_INEQUALITY_PATH.exists() and DEFAULT_FEATURES_PATH.exists()
needs_inputs = pytest.mark.skipif(not REAL_INPUTS, reason='requires the local pinned inputs')


def fake_primary(tmp_path, *, association=True, promoted=False, integrity=True):
    """A digest-consistent primary result directory (never committed, so verification must refuse)."""
...
# --- verification of the primary result ---------------------------------------------------------
...
def test_conditional_runs_for_association_support_even_without_promotion(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ev, 'build_frozen', lambda _inputs: synthetic_scoring(n=24, seed=5))
    monkeypatch.setattr(sens, 'run_arms', lambda scoring, **kwargs: (calls.append(scoring) or ({}, [])))
...
def test_station_support_arm_removes_exactly_the_five_countries(tmp_path):
    ...
    arm, reference, description = sens.station_support_arm(scoring, package_dir=tmp_path)
    assert reference is None and arm.n == scoring.n - 5
    assert not set(sens.EXCLUDED_ISO3) & set(arm.table.iso3)
    keep = ~scoring.table.iso3.isin(sens.EXCLUDED_ISO3).to_numpy()
    np.testing.assert_array_equal(arm.c2, scoring.c2[keep])
    index = np.flatnonzero(keep)
    np.testing.assert_array_equal(arm.distance, scoring.distance[np.ix_(index, index)])
...
def test_station_support_arm_refuses_a_different_five(tmp_path):
    ...
    with pytest.raises(ValueError, match='lowest support set'):
        sens.station_support_arm(scoring, package_dir=tmp_path)
...
def test_station_support_arm_compares_against_its_own_comparator(tmp_path):
    ...
    (tmp_path / 'm1b_support_checkpoint.json').write_text(json.dumps({'extremes_top10': {
        'lowest_station_supported_share': [[c, 0.1] for c in sens.EXCLUDED_ISO3]}}))
    arm, _reference, _description = sens.station_support_arm(scoring, package_dir=tmp_path)
...
def test_era5_arm_refuses_a_country_without_a_finite_aligned_outcome():
    with pytest.raises(ValueError, match='no finite aligned ERA5 outcome'):
        sens.aligned_era5_arm(synthetic_scoring(n=12, seed=2))


@needs_inputs
def test_era5_arm_replaces_only_the_outcome_with_the_committed_aligned_measurements():
    inputs = ev.verify_scoring_inputs()
    scoring = ev.build_frozen(inputs)
    arm, reference, description = sens.aligned_era5_arm(scoring)
    ...
    changed = [c for c in scoring.design.columns
               if not scoring.design[c].equals(arm.design[c])]
    ...
    assert not np.array_equal(arm.design.warming_trend.to_numpy(),
                              scoring.design.warming_trend.to_numpy())
...
def test_m1a_arm_replaces_exactly_the_five_remeasured_features():
    inputs = ev.verify_scoring_inputs()
    scoring = ev.build_frozen(inputs)
    arm, reference, description = sens.m1a_geography_arm(scoring)
    changed = [c for c in scoring.design.columns if not scoring.design[c].equals(arm.design[c])]
    ...
    assert np.array_equal(arm.c2, scoring.c2) and arm.n == scoring.n
...
def test_each_arm_records_its_own_identity_not_the_primary_run_s(tmp_path):
    scoring = scoring_with_excluded()
    scoring = dataclasses.replace(scoring, identity={**scoring.identity,
    ...
    assert identity['iso3_sha256'] != ev.hashlib.sha256(
        ('\n'.join(scoring.table.iso3) + '\n').encode()).hexdigest()
    primary = ev.hashlib.sha256(np.array(
        [t for _f, _x, t in ev.cv.iter_folds(scoring.folds[ev.PRIMARY][0], scoring.distance,
                                             ev.PRIMARY_BUFFER_KM)]).tobytes()).hexdigest()
    assert identity['primary_membership_sha256'] != primary
    assert identity['station_weights_sha256'] != ev.array_digest(scoring.station_w)
...
# --- every refusal of verified_primary, exercised ------------------------------------------------
...
def test_a_committed_pushed_unchanged_primary_result_is_accepted(tmp_path, monkeypatch):
    primary, _ = committed_primary(tmp_path, monkeypatch)
    monkeypatch.setattr(ev, '_git', git_stub())
    verified = sens.verified_primary(primary)
    assert verified['evaluator_commit'] == 'a' * 40
    assert verified['verdict']['association_supported'] is True
    assert verified['primary_scorecard_sha256'] == hydro.sha256(primary / 'm1b_scorecard.json')
...
def test_every_git_state_that_breaks_the_sequence_is_refused(tmp_path, monkeypatch, failing, message):
    primary, _ = committed_primary(tmp_path, monkeypatch)
...
def test_a_manifest_that_disagrees_with_the_result_is_refused(tmp_path, monkeypatch, overrides, message):
...
def test_a_verdict_without_a_recorded_association_boolean_is_refused(tmp_path, monkeypatch):
    primary, _ = committed_primary(tmp_path, monkeypatch)
