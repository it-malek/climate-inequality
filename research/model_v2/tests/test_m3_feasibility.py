...
def summary():
    if not (RECORD / 'm3_feasibility_manifest.json').exists():
        pytest.skip('the feasibility record is committed together with M3_FEASIBILITY_AUDIT.md')
...
def test_the_record_is_canonical_and_bound_to_pushed_code(summary):
    ...
    assert summary['code']['commit'].startswith('99f2c3d')
...
def test_the_null_boundary_frequencies_quoted_in_the_spec(summary):
