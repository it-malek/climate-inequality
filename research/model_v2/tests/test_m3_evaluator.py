...
Every fitted fixture is synthetic (the synthetic frozen frame of the M2 evaluator tests). No test fits a
...
def synthetic():
    frozen = synthetic_frozen(n=60, seed=4)
    dist = np.array(frozen.distance)
    return frozen, dist
...
def test_held_out_and_buffered_outcomes_never_enter_predictions(synthetic, family, static_model):
    frozen, dist = synthetic
    frame = frozen.frames['primary_total_co2']
    run = m3.run_family(family, static_model, frame, frozen, dist, 'primary_total_co2')
    assert run.status == 'computable', run.failure
    i = 11
    excluded = np.flatnonzero(frozen.distance[i] <= 500.0)          # i itself and its buffer
    changed = frame.copy()
...
def test_a_spatial_fit_failure_makes_the_family_non_computable_without_fallback(synthetic, monkeypatch):
    ...
    run = m3.run_family('sem', ev.COMPARATOR, frame, frozen, dist, 'primary_total_co2')
...
def test_a_boundary_estimate_is_computable_and_flagged(synthetic, monkeypatch):
    ...
    run = m3.run_family('sar', ev.COMPARATOR, frame, frozen, dist, 'primary_total_co2')
...
def test_accounting_identity_and_filtered_shares_on_synthetic_data(synthetic):
    frozen, dist = synthetic
    frame = frozen.frames['primary_total_co2']
    static = ev.score_model(ev.COMPARATOR, frame, frozen, expected_full=False)
    run = m3.run_family('sem', ev.COMPARATOR, frame, frozen, dist, 'primary_total_co2')
...
def test_representation_identity_holds_for_spatial_families_on_synthetic_identity_data(synthetic):
    ...
    for representation in ev.REPRESENTATIONS:
        ...
        for family in m3.FAMILIES:
            run = m3.run_family(family, ev.COMPARATOR, frame, frozen, dist, representation)
            runs[(representation, family)] = run
            entries[(representation, family)] = {
                'card': m3.family_card(run, frame, frozen, 'M0*', frozen.station_w, frozen.area_w, dist),
...
def test_the_code_closure_covers_the_estimator_the_static_path_and_the_frozen_documents():
    ...
    for path in ('research/model_v2/m3_evaluate.py', 'research/model_v2/m3_spatial.py',
                 ...
                 'research/model_v2/v2_provenance.py', 'research/model_v2/DOWNSTREAM_COMPLETION_SPEC.md',
...
def test_evaluate_end_to_end_on_synthetic_data(synthetic, monkeypatch, tmp_path, static, weights):
    ...
    scored = {(r, m): ev.score_model(m, frozen.frames[r], frozen, expected_full=False)
              for r in ev.REPRESENTATIONS for m in models}
    monkeypatch.setattr(m3, 'reproduce_static', lambda frozen, models, primary=None: {'record': {}, 'scored': scored})
    monkeypatch.setattr(m3, 'arm_distance', lambda frozen, weights: dist)
    result = m3.evaluate(frozen, tmp_path / 'out', weights=weights, static=static, provenance={'code': {'commit': 'x'}})
