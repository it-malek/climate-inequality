...
def synthetic_scoring(n=40, seed=0, effect=-0.04, iso3=None):
    """A scoring frame of the evaluator's shape with a synthetic outcome and synthetic C2."""
...
def reference_from(scoring, expected=None):
    """Score each comparator once so the reference-reproduction path is exercised for real."""
    arms = ev.build_arms(scoring)
    return {r: {'card': ev.score_model(arm['frames'][ev.COMPARATOR], scoring,
...
def scoring():
    return synthetic_scoring()


@pytest.fixture(scope='module')
def reference(scoring):
    return reference_from(scoring, expected=ev.EXPECTED_COLUMNS['comparator'])


@pytest.fixture(scope='module')
def evaluated(tmp_path_factory, scoring, reference):
    out = tmp_path_factory.mktemp('run') / 'primary'
    result = ev.evaluate(scoring, out, provenance={'code': {'commit': 'a' * 40}},
...
def test_arms_are_the_two_registered_representations_with_the_identical_c2(scoring):
    arms = ev.build_arms(scoring)
    ...
    for representation, arm in arms.items():
        ...
        np.testing.assert_array_equal(candidate[ev.FEATURE].to_numpy(), scoring.c2)
...
def test_full_sample_rank_expectations_hold_for_both_representations(scoring, evaluated):
    result, _ = evaluated
...
def test_a_singleton_level_gives_a_reduced_column_training_fold_and_an_unseen_level_row():
    scoring = synthetic_scoring(n=36, seed=13)
    zones = ['A'] * 12 + ['B'] * 12 + ['C'] * 11 + ['E']      # 'E' has exactly one member
    scoring = dataclasses.replace(scoring, design=scoring.design.assign(climate_zone=zones))
    candidate = scoring.design.drop(columns='cum_co2_per_capita').reset_index(drop=True)
    candidate[ev.FEATURE] = scoring.c2
    scored = ev.score_model(candidate, scoring, expected_columns=None)
    ...
    singleton = next(f for f in scored.fits if f['held_out_iso3'] == scoring.table.iso3.iloc[-1])
...
def test_both_comparators_are_scored_before_either_candidate(scoring, reference, monkeypatch):
    ...
    ev.score_representations(scoring, reference_cards=reference)
    assert order == [False, False, True, True]


def test_a_comparator_that_misses_its_reference_card_refuses(scoring, reference):
    broken = {r: {'card': {**entry['card'], 'cv_rmse': entry['card']['cv_rmse'] + 1e-9},
                  'metrics': entry['metrics']} for r, entry in reference.items()}
    with pytest.raises(ValueError, match='does not reproduce'):
        ev.score_representations(scoring, reference_cards=broken)
...
def test_support_quality_context_comes_from_the_verified_support_record():
...
def test_the_verdict_is_a_pure_function_of_the_primary_representation(evaluated):
    ...
    # Exactly one verdict is reported; the per-capita arm carries a descriptive label only.
...
def test_per_capita_arms_are_materialized_even_when_the_association_fails(tmp_path):
    scoring = synthetic_scoring(n=40, seed=3, effect=+0.05)     # a positive, wrong-signed effect
    reference = reference_from(scoring, expected=ev.EXPECTED_COLUMNS['comparator'])
    result = ev.evaluate(scoring, tmp_path / 'run', provenance={'code': {'commit': 'b' * 40}},
                         reference_cards=reference)
...
def test_saved_predictions_and_coefficients_support_independent_recomputation(evaluated, scoring):
    ...
    assert len(predictions) == 2 * 2 * 4 * scoring.n
    ...
    y = scoring.design.warming_trend.to_numpy()
    ...
    assert len(training) == scoring.n and training.held_out_iso3.nunique() == scoring.n
    folds = pd.read_csv(out / 'm1b_cv_folds.csv')
    assert len(folds) == scoring.n and folds.primary_training_iso3.str.len().min() > 0
...
def test_two_runs_of_unchanged_code_are_byte_identical(tmp_path):
    scoring = synthetic_scoring(n=28, seed=7)
    reference = reference_from(scoring, expected=ev.EXPECTED_COLUMNS['comparator'])
    kwargs = {'provenance': {'code': {'commit': 'c' * 40}}, 'reference_cards': reference}
    ev.evaluate(scoring, tmp_path / 'a', **kwargs)
    ev.evaluate(scoring, tmp_path / 'b', **kwargs)
    for name in ev.DETERMINISTIC_ARTIFACTS:
        assert (tmp_path / 'a' / name).read_bytes() == (tmp_path / 'b' / name).read_bytes()


def test_a_run_never_overwrites_a_prior_run(tmp_path, scoring, reference):
    ev.evaluate(scoring, tmp_path / 'once', provenance={'code': {'commit': 'd' * 40}},
                reference_cards=reference)
...
def test_main_refuses_before_reading_any_gate_or_input_when_the_code_is_not_committed(monkeypatch):
...
def test_a_refused_pre_scoring_check_leaves_no_output_directory(tmp_path, scoring, reference):
    broken = {r: {'card': {**entry['card'], 'in_sample_r2': entry['card']['in_sample_r2'] + 1e-6},
                  'metrics': entry['metrics']} for r, entry in reference.items()}
    out = tmp_path / 'refused'
    with pytest.raises(ValueError, match='does not reproduce'):
        ev.evaluate(scoring, out, provenance={'code': {'commit': 'e' * 40}}, reference_cards=broken)
    assert not out.exists(), 'a refusal must not poison the output path for a later retry'


def test_reproducibility_record_compares_deterministic_artifacts_only(tmp_path, scoring, reference):
...
# --- the fixed constants and the integrity path -------------------------------------------------

def test_the_pre_specified_thresholds_and_tolerances_are_the_literal_values():
    """Pinned against the literal values, not against the constants themselves."""
    from fractions import Fraction
...
def test_the_full_sample_column_and_rank_expectation_actually_fires(scoring):
    comparator = ev.build_arms(scoring)['primary_total_co2']['frames'][ev.COMPARATOR]
    with pytest.raises(ValueError, match='expected 21 / 21'):
        ev.score_model(comparator, scoring, expected_columns=ev.EXPECTED_COLUMNS['candidate'])
    candidate = ev.build_arms(scoring)['primary_total_co2']['frames'][ev.CANDIDATE]
    with pytest.raises(ValueError, match='expected 20 / 20'):
        ev.score_model(candidate, scoring, expected_columns=ev.EXPECTED_COLUMNS['comparator'])


def test_a_refused_verdict_is_recorded_as_an_integrity_failure_with_the_evidence_kept(
        tmp_path, scoring, reference, monkeypatch):
...
def test_share_accounting_failure_is_reported_as_an_integrity_failure(tmp_path, scoring, reference,
                                                                     monkeypatch):
    real = ev.score_model
    ...
    result = ev.evaluate(scoring, tmp_path / 'skewed', provenance={'code': {'commit': 'a' * 40}},
                         reference_cards=reference)
