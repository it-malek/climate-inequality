...
disclosed baseline-only anchor (M0* against its committed rank-audit cards), in which any attempt to fit M2 raises.
...
def synthetic_scoring(n=60, seed=0):
...
def test_an_altered_pin_refuses(monkeypatch):
    ...
    with pytest.raises(prov.ExecutionRefused, match='differs from the record'):
        ev.verify_pins()
...
def test_the_code_closure_is_computed_transitively():
    ...
    for path in ('research/model_v2/m2_evaluate.py', 'research/model_v2/m2_conditional.py',
                 ...
                 'research/model_v2/M2_EVALUATOR_SPEC.md',
...
def test_m0star_equals_the_v1_estimator_on_raw_values():
...
def test_cross_validated_predictions_ignore_held_out_and_buffered_outcomes():
    ...
    result, records = ev.run_protocol(ev.CANDIDATE, frame, scoring, ev.PRIMARY)
    i = 7
    buffered = np.flatnonzero(scoring.distance[i] <= 500.0)
    changed = frame.copy()
    changed.loc[buffered, OUTCOME_COL] = 1e6
    again, _ = ev.run_protocol(ev.CANDIDATE, changed, scoring, ev.PRIMARY)
    assert again.yhat[i] == result.yhat[i]
    assert len(records) == len(frame) and all(r['fitted'].state is not None for r in records)


def test_encoded_lmg_equals_the_v1_decomposition_on_raw_values():
    frame = synthetic_frame()
    raw = frame.drop(columns=['iso3', 'm49_subregion', 'station_lon', 'station_lat']).copy()
    for column in ('cum_co2_total', 'population'):
        raw[column] = 10.0 ** raw[column]
    reference = group_lmg_shares(raw)
    ours = ev.lmg_shares(ev.COMPARATOR, frame)
    for group, value in reference.shares.items():
        assert ours['shares'][group] == pytest.approx(value, abs=1e-12)
    assert ours['total_r2'] == pytest.approx(reference.total_r2, abs=1e-12)
...
def test_score_model_on_a_synthetic_frame_records_every_fit_and_gamma():
    scoring = synthetic_scoring()
    scored = ev.score_model(ev.CANDIDATE, scoring.frames['primary_total_co2'], scoring, expected_full=False)
...
def test_representations_are_equivalent_on_synthetic_identity_data():
    ...
    for representation in ev.REPRESENTATIONS:
        ...
        comparator = ev.score_model(ev.COMPARATOR, frame, scoring, expected_full=False)
        candidate = ev.score_model(ev.CANDIDATE, frame, scoring, expected_full=False)
...
def real_scoring():
    ...
    def refuse(*args, **kwargs):
        raise AssertionError('building the scoring frame must not fit anything')
...
def test_the_real_scoring_frame_passes_every_identity(real_scoring):
    identity = real_scoring.identity
...
def test_every_fit_reproduces_the_committed_feasibility_record(real_scoring, monkeypatch):
    monkeypatch.setattr(np.linalg, 'lstsq', lambda *a, **k: (_ for _ in ()).throw(AssertionError('no fits')))
    result = ev.check_against_feasibility(real_scoring)
    assert result['fits_compared'] == 364


def test_an_altered_feasibility_state_refuses(real_scoring, monkeypatch):
    original = ev.predictor_only_states

    def shifted(scoring, frame, representation):
        rows = original(scoring, frame, representation)
    ...
    with pytest.raises(prov.ExecutionRefused, match='latitude state differs'):
        ev.check_against_feasibility(real_scoring)


def test_baseline_only_anchor_m0star_reproduces_its_frozen_cards(real_scoring, monkeypatch):
    """Disclosed baseline-only anchor on real inputs; any M2 fit raises."""
    ...
    arms = ev.score_comparators(real_scoring, ev.representation_frames(real_scoring), ev.reference_cards())
    for arm in arms.values():
        assert arm['reference']['reproduced']


def test_a_pre_candidate_refusal_writes_nothing(real_scoring, monkeypatch, tmp_path):
    def refuse(scoring):
    ...
    with pytest.raises(prov.ExecutionRefused):
        ev.evaluate(real_scoring, out, provenance={}, reference_cards=ev.reference_cards())
...
def test_a_structural_arm_failure_is_non_computable_but_a_reference_failure_stops(monkeypatch):
    scoring = synthetic_scoring()
    frames = ev.representation_frames(scoring)
    ...
    entry, arms = cond.run_arm('synthetic', scoring, degenerate, reference=loose)
    ...
    with pytest.raises(cond.ArmIntegrityFailure):
        cond.run_arm('synthetic', scoring, frames, reference=strict)
