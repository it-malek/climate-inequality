"""One-ring structural-mask harmonization of C2, on synthetic CRU grids."""
import io
import json
import os
import subprocess
...
N_LAT, N_LON = 360, 720
AREA_RTOL = 1e-10
TIE_KM = 1e-6  # 1 mm; distinct neighbour candidates differ by >= ~4e-5 km on the 0.5 deg grid
...
def cells_frame(rows):
    """(country index, CRU row, CRU col, km^2) rows in the builder's sorted long format."""
...
def run(cells, native, x, n, supported=None, mean_stn=None, pure=None):
    """The harmonization pipeline on synthetic grids: every invalid support cell is a structural target."""
...
def test_nearest_wgs84_donor_is_selected():
    ...
    native_cell(native, x, 259, 100, 1.0)                   # 40.25 deg N: south (~55 km) ...
    native_cell(native, x, 260, 101, 2.0)                   # ... loses to east (~43 km)
    native_cell(native, x, 339, 300, 3.0)                   # 80.25 deg N: south (~56 km) ...
    ...
    native_cell(native, x, 99, 599, 7.0)                    # 39.75 deg S: the two western diagonals differ;
...
def test_north_and_south_neighbours_are_not_tied_on_the_ellipsoid():
    native, x, targets = empty_grids()
    targets[260, 100] = targets[100, 400] = True             # 40.25 deg N and 39.75 deg S
...
def test_longitude_wraps_across_the_antimeridian():
    ...
    native_cell(native, x, 200, 719, 0.7)                   # 179.75 deg E next to 179.75 deg W
...
def test_non_structural_invalid_support_stays_unresolved_counts_against_coverage_and_does_not_stop():
    ...
    assert h.hard_stops(qa, ['AAA']) == []                 # presence alone does not stop; it counts against coverage
...
def test_pinned_input_digests_fail_closed(tmp_path):
    good = tmp_path / 'input.bin'
    good.write_bytes(b'pinned')
    import hashlib
    h.check_frozen_inputs({good: hashlib.sha256(b'pinned').hexdigest()})
    with pytest.raises(ValueError):
...
def test_without_targets_the_harmonized_aggregate_equals_the_native_only_rule():
