"""M1b measurement-package gates: package integrity, strict gate records and outcome-free redundancy loading.

Every package here is synthetic. The instrumented redundancy run reads the pinned predictor inputs with
...
COUNTRIES = pd.read_csv(r.SUPPORT_RECORD, usecols=['iso3', 'Country'])
...
def write_package(out, c2=None, **manifest_overrides):
    ...
    values = np.linspace(-1.0, 0.5, len(COUNTRIES)) if c2 is None else c2
    texts = {h.FEATURES: COUNTRIES.assign(baseline_dryness=values).to_csv(index=False, lineterminator='\n'),
...
@pytest.mark.parametrize('mutate', [
    ...
    lambda m: m.update(gate_version='stale gate version'),
...
def test_the_preserved_stopped_build_cannot_pass_the_gate():
    with pytest.raises(ValueError):
...
def test_package_gate_accepts_only_a_redundancy_record_from_the_current_package(tmp_path):
    ...
    for overrides in [{'provenance': {'measurement_manifest_sha256': '0' * 64, 'c2_features_sha256': features_sha}},
        ...
        with pytest.raises(ValueError):
            package_gate(tmp_path)
    passing_record(tmp_path, manifest_sha, features_sha)
    write_package(tmp_path, c2=np.linspace(-2.0, 1.0, len(COUNTRIES)))   # a rebuilt package makes the record stale
    with pytest.raises(ValueError):
        package_gate(tmp_path)
...
def instrumented(monkeypatch):
    if not (r.DEFAULT_INEQUALITY_PATH.exists() and r.DEFAULT_FEATURES_PATH.exists()):
        pytest.skip('requires the local pinned predictor inputs')
...
def test_redundancy_entry_point_reads_only_allow_listed_predictor_columns(tmp_path, instrumented):
    manifest = write_package(tmp_path)
    ...
    assert record['status'] == 'pass' and record['hard_stops'] == [] and result['n'] == len(COUNTRIES)
...
def test_redundancy_refuses_before_reading_c2_or_predictors(tmp_path, instrumented, monkeypatch, case):
    overrides = {'failing_manifest': {'all_hard_stops_pass': False},
                 'old_gate_version': {'gate_version': 'stale gate version'}}.get(case, {})
...
def test_redundancy_hard_stop_is_recorded_as_valid_json(tmp_path, instrumented):
    latitude = r.m0star_predictors(COUNTRIES.Country).abs_latitude.to_numpy()
    write_package(tmp_path, c2=2.0 * latitude)                                     # exactly redundant with M0*
    result = r.main(tmp_path)
    record = json.loads((tmp_path / r.RECORD).read_text())
    ...
    write_package(tmp_path, c2=np.ones(len(COUNTRIES)))                                  # zero variance
    r.main(tmp_path)
...
def test_evaluator_main_refuses_before_any_gate_or_input_read(monkeypatch):
    """The provenance guard runs first: a wrong PYTHONHASHSEED refuses before any gate or input is read."""
