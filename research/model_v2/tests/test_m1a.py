"""M1a geography measurement rules and the M1a classification rules, on synthetic fixtures."""
import numpy as np
import pandas as pd
import pytest
import shapely
...
def test_negative_land_elevation_retained_and_negative_ocean_excluded():
    # One 0.25 deg cell at 10 to 10.25 deg N, 20 to 20.25 deg E; land ends at 20.09 deg E, inside the
    # pixel 20.0833 to 20.1 deg E whose centre (20.0917 deg E) is water.
    ...
    assert qa.elevation_coverage[0] == pytest.approx(1.0)                  # one-ring completion resolves it
...
def test_mean_absolute_latitude_differs_from_centroid_and_hemisphere_majority():
    records = g.Records()
    rr, cc = np.meshgrid(np.arange(89 * g.PIX, 92 * g.PIX), np.arange(0, 30), indexing='ij')  # -1 to +2 deg
    ...
    assert qa.abs_latitude[0] == pytest.approx((0.5 + 1 * 2 * 1.0) / 3, abs=2e-3)  # mean |lat| approximately 0.833
...
def test_lake_hierarchy_and_full_block_detection():
    ...
    country = np.full((720, 1440), -1, dtype=np.int16)
    country[360:364, 720:724] = 0  # cells covering 0 to 1 deg N, 0 to 1 deg E
    events = []
...
def test_coast_seams_removed_and_distance_wraps_dateline():
    ...
    # Point on the seam, 10 deg from the east/west edges but 5 deg from the northern shore at 10 deg N.
...
def test_m1a_classification_rules():
