...
``numpy.quantile`` knots, subset-weight Shapley shares, ``polyfit`` calibration). The fixed search itself is
taken from ``m3_independent_math.maximize`` with that separate objective. No repository data file is read, no
...
def group_blocks(x, columns):
    group = [GROUP_OF.get(c.split('=')[0], 'geography') if c != 'intercept' else 'intercept' for c in columns]
    group = ['geography' if c in ADDED else o for c, o in zip(columns, group)]
    return {g: x[:, [j for j, o in enumerate(group) if o == g]] for g in GROUPS}
