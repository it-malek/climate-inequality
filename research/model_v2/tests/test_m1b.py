...
def test_window_is_exactly_the_1920_to_1949_months_with_calendar_days():
...
# test_m1b_evaluator.py and test_m1b_scoring_gate.py.
...
def test_outcome_free_predictors_reproduce_the_m0star_predictor_columns():
    # Loads the V1 design (which contains the outcome) only to prove equivalence; no C2 is involved.
    from research.model_v2 import m0
    from research.model_v2 import m1b_redundancy as r
    if not (m0.DEFAULT_INEQUALITY_PATH.exists() and m0.DEFAULT_FEATURES_PATH.exists()):
        pytest.skip('requires the local pinned inputs')
    reference = m0.m0_complete_design(*m0.load_inputs()).drop(columns=['warming_trend', 'cum_co2_per_capita'])
    rebuilt = r.m0star_predictors(reference.Country)
    pd.testing.assert_frame_equal(rebuilt, reference.reset_index(drop=True), check_exact=True)
    np.testing.assert_array_equal(m0.design_matrix(rebuilt)[0], m0.design_matrix(reference)[0])
