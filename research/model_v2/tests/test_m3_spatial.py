"""M3 explicit spatial structure: graphs, SEM/SAR ML, prediction, uncertainty, accounting (spec §2.2-§2.7, §2.11).
...
# --- the frozen maximizer (§2.5) ---------------------------------------------------------------

def _replay_golden(objective, k):
    """Independent spelling of spec §2.5 step 3; returns the golden-stage points in evaluation order."""
...
@pytest.mark.parametrize('family', ['sem', 'sar'])
def test_a_fit_records_the_frozen_evidence(cloud, fits, family):
    _, w, x = cloud
    y, fit = fits[family]
...
# --- fitted quantities (§2.6) -------------------------------------------------------------------

@pytest.mark.parametrize('family', ['sem', 'sar'])
def test_fitted_quantities_follow_the_frozen_definitions(cloud, fits, family):
    _, w, x = cloud
    y, fit = fits[family]
