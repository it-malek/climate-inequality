"""M3 independent reference mathematics (DOWNSTREAM_COMPLETION_SPEC.md §2.2-§2.7, §2.11).
...
def test_evaluation_failures_are_named_in_the_frozen_order():
...
def test_the_search_is_the_frozen_grid_then_sixty_golden_iterations():
    target = 0.3137
...
def test_material_change_flags_follow_the_plan_definition():
...
@pytest.mark.parametrize('family', FAMILIES)
def test_theta_se_is_the_inverse_information_element_with_the_frozen_wald_quantile(family):
