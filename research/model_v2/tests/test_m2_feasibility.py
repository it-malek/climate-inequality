...
def synthetic_predictors(n=48, seed=0):
    """Transformed numerics, as the audit reads them from the frozen encoded design."""
...
def test_the_audit_pins_are_the_frozen_evaluator_pins():
...
def test_the_committed_feasibility_record_matches_its_code_and_its_manifest():
    ...
    document_tracked = subprocess.run(['git', 'ls-files', '--error-unmatch', 'research/model_v2/M2_FEASIBILITY_AUDIT.md'],
                                      cwd=fe.ROOT, capture_output=True).returncode == 0
    if not summary_path.exists():
        assert not document_tracked, 'M2_FEASIBILITY_AUDIT.md is committed but its record is missing'
        pytest.skip('the feasibility record is committed together with M2_FEASIBILITY_AUDIT.md')
    ...
    for path, digest in code['sha256'].items():
        assert fe.sha256(fe.ROOT / path) == digest, f'{path} changed after the feasibility record was made'
