"""Country geometry from the GPW v4 national-identifier grid, ETOPO 2022 and Koppen-Geiger.
...
elevation statistics and area-weighted Koppen major-group shares. Nothing here is
a model input; the V1 design is untouched.

All grids are the ones V1 already relies on (``src.area_weighting`` uses the same
GPW band for country assignment; ``src.explain`` samples the same ETOPO and Koppen
files at station coordinates). Resolution: GPW 15 arc-minutes, ETOPO sampled every
fifth 60 arc-second row/column (5 arc-minutes), Koppen 0.5 degrees.
"""
...
def koppen_area_shares(
    ...
    """Area-weighted share of each Koppen major group per country, and the dominant one."""
    ...
    try:
        ...
        ds.close()
    # Look up the 0.5 deg Koppen cell that contains each 15' GPW cell centre.
