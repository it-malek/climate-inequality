...
Two responsibilities, side effects only at these edges:
  ...
  2. Load ``GlobalLandTemperaturesByCity.csv`` into a DuckDB database,
     ...
     ingestion so downstream stages only ever see signed-float lat/lon.
"""
...
def write_typed_parquet(
    ...
    Shared writer for the pipeline's output tables (trends, inequality):
...
def load_city_temperatures(
    ...
    preserved; windowing and coverage filters belong to later stages.
