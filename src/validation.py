...
1. **Overlap agreement** (:func:`overlap_agreement`): per city-location,
   ...
   1 x 1 degree grid cell over the analysis window. Cities whose cell does not
   track them (islands and coastlines, where a 1-degree land average is not
...
``temperature(time, latitude, longitude)`` float32 anomalies in deg C,
...
# (Gridded -> Monthly Land -> 1 x 1 degree).
GRIDDED_URL = (
...
def acceleration(
    ...
    Returns:
        ...
        slope_overlap_grid, slope_delta (deg C/decade).
    """
    months = grid_wide.index
