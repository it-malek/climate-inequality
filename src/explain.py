...
def normalize_longitudes(lons: np.ndarray, convention: str) -> np.ndarray:
    ...
    :func:`src.grids.nearest_cell_indices` compares like with like.
...
def sample_static_grid(
    ...
    Shared by elevation (:func:`add_elevation`) and Koeppen class
    (:func:`add_koppen`) sampling. Longitudes are normalized to the grid's
    convention (:func:`normalize_longitudes`,
    :func:`check_coordinate_orientation`) before the nearest-cell lookup
    (:func:`src.grids.nearest_cell_indices`), so a [-180,180]-vs-[0,360]
    mismatch cannot silently wrap to the wrong side of the globe.
