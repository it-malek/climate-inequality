"""Area-weighted country warming from the Berkeley Earth 1-degree gridded field.
...
Earth 1 x 1 degree grid (the same estimator, time axis and 1950-01..2013-09 window
as the station pipeline, so only the weighting differs), each cell assigned to a
...
# Berkeley Earth 1 x 1 degree gridded monthly anomalies (gitignored, ~199 MB; the
# same file src.validation reads).
...
# ~ chunk x n_lon x n_window_months float32 -- a few MB at 1-degree resolution).
...
def cell_trends(
    ...
    *,
    ...
    the input may be absolute temperature or anomalies. A constant Kelvin-to-Celsius
    offset leaves slopes unchanged, but subtracting twelve calendar-month
    climatologies can change a monthly Theil-Sen slope. The preferred ERA5
    product comparison uses monthly anomalies (research.model_v2.product_stability).
    ...
    Returns:
        ...
        ``(n_lat, n_lon)`` float array of deg C/decade slopes (NaN where unfit).
...
def world_land_mean(nc_path: Path = BERKELEY_GRID_PATH, **kwargs) -> float:
    ...
    should sit near the station global mean (~0.146 deg C/decade) -- a modest
