...
projections. Each entry (a :class:`ProjectionContract`) carries a SHA-256
``definition_hash`` over its fields, mirrored into ``docs/pcs_v1.yaml`` /
``docs/pcs_v2.yaml`` (regenerated by ``python -m src.pcs``), so an edit to a
definition shows up in the mirror's diff. A revised definition is a new name or
version, never an in-place edit; names may not carry variant suffixes such as
``_adjusted`` or ``_corrected``.
"""
...
def to_yaml(
    ...
    header: str = "# Generated mirror of src.pcs.PCS_V1 (the projection registry).",
) -> str:
    """Serialize a registry to a deterministic YAML string (the doc mirror).

    The Python dataclasses are the in-repo source of truth; this is a diff
    artifact (including each contract's ``definition_hash``), regenerated by
...
_PCS_V2_HEADER = "# Generated mirror of src.pcs.PCS_V2 (the wide projection registry)."
