...
structural axes of the fixed feature schema
...
Population/area weighting is a documented degree of freedom the stability
diagnostics perturb, not a default -- the headline inequality is *between
countries as units*.
"""
...
def variance(values: np.ndarray | pd.Series, weights=None) -> float:
    ...
    squared deg C/decade. Population variance (not sample) because the target is
    the observed set of countries, not a superpopulation.
    """
    x, w = _as_arrays(values, weights)
