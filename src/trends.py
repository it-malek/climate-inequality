...
(deg C/decade, 95% CI) plus an OLS slope per location. Heavy filtering and
...
def compute_climatology(
    ...
    Returns:
        ...
        with a `climatology` column (deg C). Months with no non-null baseline
...
def compute_anomalies(
    ...
    Returns:
        ...
        `anomaly` column (deg C), for kept locations only.
    """
...
def fit_city_trends(
    ...
    """Theil-Sen and OLS warming trends per city-location, in deg C/decade.

    Time is expressed in decimal decades, so fitted slopes are directly in
    deg C/decade; `ci_low`/`ci_high` are the Theil-Sen 95% confidence bounds.
