...
recomputed on a perturbed sample, so these diagnostics can only ever speak to
the **stability of the variance shares**, never to significance or causation.
Like the decomposition itself, this is a *descriptive variance attribution of
observed warming trends*, not climate attribution; the canonical disclaimer
...
The input is the schema-named design from
:func:`src.decomposition.build_country_design`, so this module never sees a
feature outside :data:`SCHEMA_V1`; the schema is perturbed in *sampling*, never in
membership. See ``docs/stability.md``.

The bundle carries the stability summary twice: for the primary, area-weighted
outcome (with all three blocks) and, under a ``sensitivity`` key, for the
...
def _share_stability(
    ...
    *,
    ...
    # the unexplained remainder, not an axis.
    n_ok = len(boot_rows)
    p_geo_largest = None
...
def build_stability_summary(
    ...
    *,
    ...
    Args:
        ...
        schema: the feature schema (defaults to :data:`SCHEMA_V1`); membership
