"""Feature schema v1: the fixed feature representation of the decomposition.
...
This is a strictly cross-sectional, *descriptive* decomposition of **observed**
warming trends. It contains no physical/driver (time-series climate-attribution)
model: such a model's output would be spatially constant across countries and so
explain none of the cross-country variance, and the project lacks the
aerosol/ENSO/volcanic data it needs. :data:`INTERPRETATION_NOTE` is the canonical
disclaimer that travels with every model output, and every module imports it
rather than restating it. See ``docs/decomposition_design_memo.md``.
...
Maturity
--------
:data:`SCHEMA_V1` carries ``maturity="candidate"``: the enforcement is strict
(nothing outside the declared schema reaches a model), but a later revision may
still adjust grouping or feature membership, so downstream code should not
assume v1's group boundaries are final. A revision is a **new**
:data:`FeatureSchema` (``"v2"``) added to :data:`SCHEMAS`; an immutable
``SCHEMA_V*`` object is never mutated in place.
...
- **residual is named**, so per-group shares plus residual always sum to 1 and
  the unexplained part is a testable object (the stability diagnostics probe
...
- LMG/Shapley attributes R^2 to **groups**, not individual features. The
  ...
  membership is part of the schema, not a model-time choice.
...
- ``status="proposed"`` features are excluded from the live design matrix
  ...
  is wired -- adding a source later is a visible schema change, not a silent
...
# are part of the schema but excluded from models until their source lands.
...
# may still shift in a later revision; "frozen" => fixed for downstream use.
...
class FeatureSpec:
    """One feature in the schema, described at the definition level.

    ``description`` says *what the feature measures*, deliberately not how it
    is computed or sourced -- this is a representation schema, not an ETL
    recipe.
    """
...
# ---------------------------------------------------------------------
# Lookups and schema enforcement
# ---------------------------------------------------------------------
...
def validate_design_matrix(
    ...
    *,
    ...
    """Enforce the schema: every design-matrix column must be a schema feature.

    The hard rule of the feature representation -- nothing outside the schema
    ...
    Args:
        ...
        schema: schema to check against (defaults to :data:`SCHEMA_V1`).
    ...
    if unknown:
        raise ValueError(
            ...
            "-- no feature outside the schema may enter a model"
        )
...
def to_yaml(schema: FeatureSchema = SCHEMA_V1) -> str:
    ...
    The Python dataclasses are the single source of truth; this is a diff
    artifact, regenerated by :func:`main`.
    """
    lines = [
        "# Generated mirror of src.feature_schema.SCHEMA_V1 (the feature schema).",
        "# Canonical source is the Python dataclasses; regenerate with:",
...
def main() -> None:
    ...
    out = write_yaml()
    print(f"Feature schema {SCHEMA_V1.version} ({SCHEMA_V1.maturity}), "
